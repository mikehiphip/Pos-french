...
class TableDetailController extends Controller
{
    ...
    protected $name_page = "Table Detail";
    ...
    public function datatable(Request $request){
        ...
        $like = $request->Like;
        $sTable = TableDetailModel::orderby('id','asc')
        ->when($like, function ($query) use ($like) {
            if (@$like['name'] != "") {
                $query->where('tb_name', 'like', '%' . $like['name'] . '%');
            }
        })
        ->get();
    ...
    {
        $datas = TableDetailModel::find($id);
        if (@$datas) {
            $query = TableDetailModel::destroy($datas->id);
        }
    ...
    public function store($request, $id = null)
    {
        try {
            DB::beginTransaction();
            if ($id == null) {
                $data = new TableDetailModel();
                $data->created_at = date('Y-m-d H:i:s');
                $data->updated_at = date('Y-m-d H:i:s');
            } else {
                $data = TableDetailModel::find($id);
                $data->updated_at = date('Y-m-d H:i:s');
            }
